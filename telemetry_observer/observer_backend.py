    ...
    async def _poll_dlq_size(self):
        while self.running:
            try:
                total = 0
                if DLQ_PATH.exists():
                    # DLQ is one JSONL file per day (one line per rejected
                    # event) — counting FILES instead of LINES undercounts
                    # the real backlog by orders of magnitude (stays 0 or 1
                    # almost always, even with thousands of rejected rows),
                    # hiding exactly the failure this metric exists to surface.
                    for fp in DLQ_PATH.glob("*.jsonl"):
                        try:
                            with fp.open("r", encoding="utf-8") as fh:
                                total += sum(1 for _ in fh)
                        except OSError:
                            pass
                self.global_dlq_size = total
            except Exception:
                pass
