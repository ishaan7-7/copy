    ...
    while True:
        ...
        for mod in config.ENABLED_MODULES:
            ...
            try:
                ...
                files = dr.list_files(mod_path, max_files=200)
                if not files:
                    continue
        ...
        # errors="coerce": a single malformed timestamp (e.g. contaminated or
        # inconsistently-formatted source data) must never crash the whole
        # engine — pd.to_datetime's default strict mode raises on the entire
        # batch for one bad row, and since checkpoints only advance on a
        # successful cycle, an uncaught crash here reprocesses (and re-crashes
        # on) the exact same poisoned row forever, permanently blocking Gold.
        parsed_ts = pd.to_datetime(combined_df["timestamp"], errors="coerce", utc=True)
        bad_mask = parsed_ts.isna()
        if bad_mask.any():
            bad_sims = sorted(combined_df.loc[bad_mask, "source_id"].unique().tolist())
            bad_samples = combined_df.loc[bad_mask, "timestamp"].head(3).tolist()
            print(f"WARNING: dropping {int(bad_mask.sum())} row(s) with unparseable timestamp "
                  f"(source_id={bad_sims}, samples={bad_samples})")
            combined_df = combined_df.loc[~bad_mask].copy()
            parsed_ts = parsed_ts.loc[~bad_mask]
        if combined_df.empty:
            time.sleep(config.POLL_INTERVAL)
            continue
        combined_df["timestamp"] = parsed_ts
        ...
        # Defense in depth: any other unexpected data-quality issue in this
        # cycle's batch (missing field, bad dtype, etc.) must be visible and
        # skippable, not a silent or fatal failure — checkpoints haven't been
        # committed yet, so skipping just retries this batch next cycle
        # instead of taking the whole engine down with it.
        try:
            ...
            print(f"WARNING: aggregation failed for this cycle, skipping ({e})")
            time.sleep(config.POLL_INTERVAL)
            continue
