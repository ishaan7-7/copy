...
echo   Installing pipeline dependencies (this can take a few minutes)...
"%ROOT%\.venv\Scripts\pip.exe" install -r "%ROOT%\requirements.txt"
if errorlevel 1 (
    echo.
    echo ERROR: pip install failed for requirements.txt — see the output above for
    echo the specific package that failed. Common causes: no internet connection,
    echo or a package with no prebuilt wheel for your Python version ^(check the
    echo Python version printed in STEP 0 above against what requirements.txt was
    echo built against^).
    echo Fix the issue above, then re-run setup.bat — completed steps are skipped.
    pause
    exit /b 1
)
...
echo   Installing dashboard dependencies (this can take a few minutes)...
"%ROOT%\master_dashboard\.venv_dash\Scripts\pip.exe" install -r "%ROOT%\master_dashboard\requirements.txt"
if errorlevel 1 (
    echo.
    echo ERROR: pip install failed for master_dashboard\requirements.txt — see the
    echo output above for the specific package that failed.
    echo Fix the issue above, then re-run setup.bat — completed steps are skipped.
    pause
    exit /b 1
)
...
if exist "%ROOT%\kafka\config\server.properties" (
    ...
    "%ROOT%\.venv\Scripts\python.exe" -c "p='%ROOT%\\kafka\\config\\server.properties'.replace('\\\\','/'); content=open(p).read(); f=open(p,'a'); f.write('\nlog.cleaner.enable=false\n') if 'log.cleaner.enable' not in content else None; f.close()"
    echo   server.properties patched: log.cleaner.enable=false (Windows log-cleaner file-lock crash workaround)
)
...
"%ROOT%\.venv\Scripts\python.exe" -c "import psutil, fastapi, pandas, duckdb, deltalake, aiokafka" 2>nul
if errorlevel 1 (
    echo   ERROR: .venv is missing one or more required packages.
    echo     Fix: "%ROOT%\.venv\Scripts\pip.exe" install -r "%ROOT%\requirements.txt"
    set VERIFY_FAILED=1
) else (
    echo   .venv: OK
)

"%ROOT%\master_dashboard\.venv_dash\Scripts\python.exe" -c "import fastapi, pandas, duckdb" 2>nul
if errorlevel 1 (
    echo   ERROR: .venv_dash is missing one or more required packages.
    echo     Fix: "%ROOT%\master_dashboard\.venv_dash\Scripts\pip.exe" install -r "%ROOT%\master_dashboard\requirements.txt"
    set VERIFY_FAILED=1
) else (
    echo   .venv_dash: OK
)
...
if not exist "%ROOT%\tools\node\node.exe" (
    echo   ERROR: tools\node\node.exe not found ^(Node.js extraction did not complete^).
    set VERIFY_FAILED=1
) else (
    echo   Node.js: OK
)

if not exist "%ROOT%\master_dashboard\frontend\node_modules" (
    echo   ERROR: frontend node_modules not found ^(npm install did not complete^).
    set VERIFY_FAILED=1
) else (
    echo   Frontend dependencies: OK
)

if not exist "%ROOT%\data\vehicles" (
    echo   WARNING: data\vehicles\ not found — unzip the separately-shipped
    echo     setup.zip ^(vehicles + extras^) into "%ROOT%" before running the app.
) else (
    dir /b "%ROOT%\data\vehicles" 2>nul | findstr "." >nul
    if errorlevel 1 (
        echo   WARNING: data\vehicles\ exists but is empty — unzip the
        echo     separately-shipped setup.zip into "%ROOT%" before running the app.
    ) else (
        echo   Vehicle CSVs: OK
    )
)
...
if "!VERIFY_FAILED!"=="1" (
    echo.
    echo ============================================
    echo  SETUP INCOMPLETE — see ERROR lines above
    echo ============================================
    echo  Fix the issue^(s^) above, then re-run setup.bat.
    echo  Completed steps are skipped automatically, so it's safe to re-run.
    pause
    exit /b 1
)
...
echo.
echo     No environment variables to set and nothing else to configure —
echo     run.py and tools\kafka\*.bat already point at C:\jdk-11.0.28+6
echo     themselves for every process they launch.
echo.
echo  3. Start the emulator:
echo     python run.py --start
echo     Then start replay from notebooks\002_replay_controller_05-01-2026.ipynb
echo.
echo ============================================
pause