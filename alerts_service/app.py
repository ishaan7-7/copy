...
def main():
    ...
    while True:
        ...
        # errors="coerce": a single malformed timestamp must never crash the
        # whole engine — pd.to_datetime's default strict mode raises on the
        # entire batch for one bad row, and since checkpoints only advance on
        # a successful cycle, an uncaught crash here reprocesses (and
        # re-crashes on) the exact same poisoned row forever, permanently
        # killing the alerts pipeline. Mirrors gold_service/app.py's handling
        # of the same risk.
        ...
        if bad_mask.any():
            ...
            print(f"WARNING: dropping {int(bad_mask.sum())} row(s) with unparseable timestamp (source_id={bad_sims})")
        ...
        if alert_updates:
            try:
                ...
                # Checkpoints haven't been committed yet, so skipping here
                # just retries this batch next cycle instead of taking the
                # whole engine down with it (same principle as gold_service).
                print(f"WARNING: alert write failed, skipping this cycle ({e})")
                time.sleep(config.POLL_INTERVAL)
                continue
