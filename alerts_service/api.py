...
# A hard ceiling on top of alert_engine.py's static per-module surface
# probability. That gate only reduces the RATE new alerts get generated at
# — it can't guarantee a proportion, since a fixed percentage of a much
# larger underlying candidate count can still end up comparable to (or
# bigger than) 100% of a small one. This caps each restricted module's
# SHARE of the currently-OPEN fleet-wide set, evaluated fresh every refresh
# cycle against whatever the real current mix is, so the dashboard can't
# end up dominated by any one non-critical-system module regardless of how
# the underlying ML-flagged rate happens to be distributed.
_MODULE_OPEN_ALERT_SHARE_CAP = {
    "battery": 0.25,
    "body": 0.12,
    "tyre": 0.12,
}
...
def _apply_module_balance_cap(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty or "module" not in df.columns:
        return df

    uncapped_df = df[~df["module"].isin(_MODULE_OPEN_ALERT_SHARE_CAP)]
    u = len(uncapped_df)
    total_share = sum(_MODULE_OPEN_ALERT_SHARE_CAP.values())

    # Sizing each cap off the RAW total only holds up while engine/
    # transmission (uncapped) are the majority — if the underlying
    # candidate pool happens to be inverted (mostly body/tyre, few
    # engine/transmission), a percentage of that inflated raw total is
    # still too generous. Solving for the post-cap total directly (holding
    # the uncapped count U fixed, since those are never touched) is what
    # actually guarantees each restricted module's target share of the
    # FINAL displayed set: target_total = U / (1 - sum of capped shares).
    if u > 0 and total_share < 1.0:
        target_total = u / (1.0 - total_share)
    else:
        # No uncapped alerts to anchor against — fall back to the raw
        # total so a fleet with zero engine/transmission alerts doesn't
        # get its entire feed zeroed out by a divide-by-near-zero blowup.
        target_total = len(df)

    kept_parts = [uncapped_df]
    for module, share in _MODULE_OPEN_ALERT_SHARE_CAP.items():
        mod_df = df[df["module"] == module]
        if mod_df.empty:
            continue
        cap = int(target_total * share)
        # mod_df inherits df's own peak_anomaly_ts-descending sort, so
        # head(cap) keeps this module's most recent alerts and drops the
        # oldest excess first.
        kept_parts.append(mod_df.head(cap))
    return pd.concat(kept_parts, ignore_index=True).sort_values("peak_anomaly_ts", ascending=False)
...
def _sync_update_alerts():
    ...
    try:
        ...
        if not df_alerts.empty:
            ...
            open_df = _apply_module_balance_cap(df_alerts[df_alerts['status'] == "OPEN"]).head(200)
...
if __name__ == "__main__":
    ...
    uvicorn.run("api:app", host="127.0.0.1", port=8004)