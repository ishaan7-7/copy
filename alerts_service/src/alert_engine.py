...
# A second, independent gate on top of the ML-driven leaky bucket above.
# Real fleets don't get bombarded with body/tyre alerts at the same rate as
# engine/transmission ones — those systems get monitored closely because
# failures are expensive and safety-critical; body/tyre issues are usually
# caught in routine inspection instead. This doesn't change whether the ML
# pipeline correctly detected a fault (the bucket logic above is untouched),
# it decides whether an individual alert INSTANCE gets surfaced to the
# dashboard once ML has already flagged it, so the overall alert mix reads
# as realistic instead of uniform across all five modules.
_MODULE_ACCEPT_PROBABILITY = {
    "engine": 1.0,
    "transmission": 0.9,
    "battery": 0.55,
    "body": 0.12,
...
_SEVERITY_OVERRIDE: dict[tuple, str] = {}
    ...
    def process_row(self, row):
        ...
        if state["phase"] == "IDLE":
            if state["fault_score"] >= config.MAX_FAULT_SCORE:
                ...
                # Rolled once per alert instance, not per row — otherwise a
                # persistently-critical module would just keep re-rolling
                # every tick while pinned at MAX_FAULT_SCORE and surface
                # almost certainly anyway, defeating the point of the gate.
                state["surfaced"] = random.random() < _MODULE_ACCEPT_PROBABILITY.get(module, 1.0)
                if state["surfaced"]:
