...
with open(os.path.join(_ROOT, "config", "fleet_manifest.json"), encoding="utf-8") as _fh:
    ACTIVE_IDS: list[str] = json.load(_fh)["active_sims"]
...
def main() -> None:
    ...
    parser.add_argument("--root", help="Expanded data root directory")
    parser.add_argument("--data-root", help="Alias for --root")
    parser.add_argument("--yes", action="store_true", help="Skip confirmation prompt")
    parser.add_argument("--dry-run", action="store_true", help="Print what would happen without writing")
    args = parser.parse_args()

    _root_str = args.root or getattr(args, "data_root", None)
    if not _root_str:
        parser.error("--root or --data-root is required")
    root = os.path.abspath(_root_str)
    ...
    for target_id in ACTIVE_IDS:
        ...
        for module in MODULES:
            ...
            # Contract format is "YYYY-MM-DD HH:MM:SS+00:00" (no fractional
            # seconds, explicit UTC offset) — every consumer downstream
            # (replay's validator, gold's pandas parsing) expects this exact
            # shape uniformly. A prior version stripped the tz and truncated
            # to milliseconds here, producing a format nothing else in the
            # pipeline recognized.
            df["timestamp"] = new_ts.strftime("%Y-%m-%d %H:%M:%S+00:00")
            if "date" in df.columns:
                df["date"] = new_ts.strftime("%Y-%m-%d")
