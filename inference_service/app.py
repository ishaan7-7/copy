...
def run_module(module, state, ml, reader, writer):
    ...
    for sim_id in active_sims:
        ...
        try:
            ...
            if not out_df.empty:
                ...
                checkpoint_updates[sim_id] = str(sim_df['ingest_ts'].max())
    ...
    try:
        writer.write(combined, module)
    except Exception as e:
        print(f"Warning: Silver write failed for {module}, will retry next cycle: {e}")
        return False

    for sim_id, max_ingest_ts in checkpoint_updates.items():
        state.update_checkpoint(sim_id, max_ingest_ts, flush=False)
    state.flush_checkpoints()
    return True
