...
class StateManager:
    ...
    def update_checkpoint(self, sim_id, timestamp_str, flush=True):
        key = f"{sim_id}_{self.module}"
    ...
    def update_ml_state(self, sim_id, ema_error, persistence_counter, last_window_data):
        ...
        if len(self.ml_state) > 100:
            keys = list(self.ml_state.keys())
            for k in keys[:len(keys) - 100]:
                del self.ml_state[k]
        self._pkl_write_counter += 1
        if self._pkl_write_counter >= 5:
