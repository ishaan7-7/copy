...
class MLEngine:
    def __init__(self, state_manager, module):
        ...
        self.models = {}
        self._floor = _HEALTH_FLOOR.get(module, 70)
        self._load_artifact()

    def _rescale_health(self, raw: float, vehicle_id: str = "") -> float:
        clamped = min(max(raw, 0.0), 100.0)
        scaled = self._floor + (clamped / 100.0) * (100 - self._floor)
        bias = _VEHICLE_HEALTH_BIAS.get(vehicle_id, 1.0)
        return min(100.0, max(0.0, scaled * bias))
    ...
    def process_batch(self, df_batch, sim_id):
        ...
        # Missing columns AND columns known to be miscalibrated against the
        # frozen model (config.EXCLUDED_ERROR_FEATURES) are both masked out of
        # the reconstruction error the same way — neither should influence
        # composite_score/health_score/severity or be reportable as a top
        # anomaly driver.
        excluded_cols = set(config.EXCLUDED_ERROR_FEATURES.get(self.module, [])) & set(expected_cols)
        mask_cols = set(missing_cols) | excluded_cols
        mask_indices = [expected_cols.index(c) for c in mask_cols]
