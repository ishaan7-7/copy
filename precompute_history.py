"""
Precomputes trip/event/DTC history for the 15 historical (non-active)
vehicles using route geometry.
...
from route_data import load_route, DenseRoute, US_ROUTES_DIR  # noqa: E402
from common.driver_score import compute_driver_score  # noqa: E402

MODULES = ["engine", "transmission", "battery", "body", "tyre"]

# compute_gold_historical.py (run earlier in the same setup_test_device.py
# pipeline) computes a REAL health time-series for every historical vehicle
# from its actual batch-processed silver data, using the same weighted
# aggregation as the live gold service. That is the vehicle's genuine
# last-known health — fleet_config.py's "health" field is only a
# hand-authored seed number used to *generate* the synthetic trip/DTC data
# below, not a measurement. Preferring the real batch-gold value here (when
# it exists) keeps this vehicle's last-known state consistent with the same
# number the live pipeline would have converged on, instead of silently
# duplicating the static seed as if it were a computed result.
BATCH_GOLD_VEHICLE_HEALTH_ROOT = os.path.join(
    os.path.dirname(__file__), "..", "data", "batch", "gold", "vehicle_health"
)


def _load_last_batch_gold(vid: str) -> dict | None:
    path = os.path.join(BATCH_GOLD_VEHICLE_HEALTH_ROOT, f"{vid}.parquet")
    ...
    try:
        ...
        return None
    if df.empty or "gold_window_ts" not in df.columns:
        return None
    last = df.sort_values("gold_window_ts").iloc[-1]
    module_health = {}
    for mod in MODULES:
        col = f"{mod}_contrib"
        if col in last and pd.notna(last[col]):
            module_health[mod] = float(last[col])
    if not module_health:
        return None
    return {
        "health": float(last["vehicle_health_score"]),
...
def _driver_score(events: list[dict], total_km: float) -> float:
    ...
    return compute_driver_score(braking, accel, cornering, total_km)
...
def _extract_segment(route_points: list, start_km: float, end_km: float, max_pts: int = 3000) -> list[tuple]:
...
def _process_vehicle(v: dict, computed_root: str, region: str = "india") -> None:
    vid = v["id"]
    seed_key = vid if region == "india" else f"{vid}|{region}"
    seed = int(hashlib.md5(seed_key.encode("utf-8")).hexdigest(), 16) % (2 ** 31)
    rng = np.random.default_rng(seed)

    print(f"  {vid}  status={v['status']}  health={v['health']}")

    route_key = v.get("route_us") if region == "america" else v.get("route")
    driver_name = v.get("driver_us", v["driver"]) if region == "america" else v["driver"]
    if not route_key:
        print(f"    SKIP: no route defined for region={region}")
        return

    data_end_ts = pd.Timestamp("2024-07-14T00:00:00", tz="UTC")
    data_start_ts = data_end_ts - pd.Timedelta(days=30)

    route = load_route(route_key, US_ROUTES_DIR) if region == "america" else load_route(route_key)
    ...
    real_gold = _load_last_batch_gold(vid)
    if real_gold is not None:
        health = real_gold["health"]
        module_health = real_gold["module_health"]
        health_source = "batch-gold (real, last computed)"
    else:
        health = v["health"]
        module_health = v.get("module_health", {})
        health_source = "fleet_config (static seed, no batch-gold data found)"
    print(f"    health source: {health_source} -> {health}")
    ...
    if v["status"] != "active":
        if region == "america":
            # fleet_config only carries one static parked-position (India) per
            # vehicle — for the US region there is no equivalent, so place the
            # vehicle at its assigned US route's destination instead of
            # leaking the Indian lat/lng into an America-mode map.
            last_lat, last_lng = route.points[-1].lat, route.points[-1].lng
            last_city = route.destination
        else:
            last_lat, last_lng = v.get("lat", 0.0), v.get("lng", 0.0)
            last_city = v.get("city", "")
        last_state = {
            "vehicle_id": vid,
            "name": vid,
            ...
            "heading": 0.0,
            "health": health,
            # Only reuse fleet_config's static composite score alongside its
            # static health — once health comes from real batch-gold data,
            # composite must be re-derived from THAT number, or the two
            # fields would silently disagree (a stale composite computed
            # against the old static health, paired with the new real one).
            "composite_score": round(1.0 - health / 100.0, 3) if real_gold is not None else v.get("composite", round(1.0 - health / 100.0, 3)),
            "module_health": module_health,
            "driver": driver_name,
            "driver_score": score,
            "last_data_ts": data_end_ts.isoformat(),
            "road_type": "",
            "route_name": last_city,
...
def main() -> None:
    ...
    parser.add_argument("--region", default="india", choices=["india", "america"], help="Route region to generate trips for")
    parser.add_argument("--yes", action="store_true", help="Skip confirmation prompt")
    args = parser.parse_args()

    computed_root = os.path.abspath(args.computed_root)
    historical_vehicles = [v for v in VEHICLES if v["status"] != "active"]

    print(f"\nComputed root: {computed_root}")
    print(f"Region       : {args.region}")
    print(f"Vehicles     : {len(historical_vehicles)} historical (of {len(VEHICLES)} total)\n")
    ...
    for v in historical_vehicles:
        _process_vehicle(v, computed_root, region=args.region)
