...
function deduplicateAlerts(alerts: any[]): any[] {
  ...
  for (const alert of alerts) {
    ...
    if (
      !existing ||
      new Date(alert.peak_anomaly_ts) > new Date(existing.peak_anomaly_ts)
    ) {
      map.set(key, alert);
    }
  }
...
export default function DtcInvestigation() {
  ...
  const fleetDistributionQuery = useQuery({
    ...
    queryFn: () =>
      axios
        .get(`${API}/api/automotive/dtc/fleet-distribution`)
        .then((r) => r.data),
    staleTime: 30000,
  ...
  const allHistoryQuery = useQuery({
    ...
    queryFn: () =>
      axios.get(`${API}/api/automotive/dtc/history`).then((r) => r.data),
    staleTime: 30000,
    refetchInterval: 10 * 60 * 1000,
  });
  ...
  const alertCoverage = useMemo(() => {
    ...
    return {
      ...
      remaining: deduped.length - analyzed,
    };
  ...
  const perVehicleSummary = useMemo(() => {
    ...
    runs.forEach((r: any) => {
      if (!map.has(r.source_id))
        map.set(r.source_id, { runCount: 0, codes: new Map(), lastRunTs: "" });
      ...
      (r.triggers || []).forEach((t: any) => {
        ...
        if (c) {
          ...
          v.codes.set(t.code, { count: 1, severity: t.severity || "" });
        }
      });
    });
    return Array.from(map.entries())
      .map(([vehicle_id, v]) => {
        ...
        return {
          vehicle_id,
          runCount: v.runCount,
          uniqueCodes: v.codes.size,
          topCode,
          topSev,
          topCnt,
          lastRunTs: v.lastRunTs,
        };
      })
  ...
  const fleetKpis = useMemo(() => {
    ...
    runs.forEach((r: any) => {
      (r.triggers || []).forEach((t: any) => {
        allCodes.add(t.code);
        if (t.severity === "critical" || t.severity === "CRITICAL")
          criticalCount++;
      });
    });
    return {
      totalRuns: runs.length,
      uniqueFaults: allCodes.size,
      criticalCount,
      topCode: distributionData[0]?.code || "—",
    };
  ...
  const evidenceOption = useMemo((): EChartsOption => {
    ...
    if (peakTsMs && xData.length > 0) {
      ...
      if (s < e)
        markAreaData.push([
          { xAxis: xData[s], itemStyle: { color: "rgba(211,47,47,0.08)" } },
          { xAxis: xData[e] },
        ]);
      if (peakIdx >= 0)
        markPointData.push({
    ...
    return {
      title: {
        text: selectedSensor.replace(/_/g, " ").toUpperCase(),
        textStyle: { fontSize: 11, fontFamily: "monospace", color: ct.axisColor, fontWeight: "bold" },
        left: 50,
        top: 2,
      },
      tooltip: {
      ...
      grid: { top: 28, right: 16, bottom: 52, left: 50 },
      xAxis: {
        type: "category",
        data: xData,
        axisLabel: {
      ...
      yAxis: {
        ...
        axisLine: { show: false },
        axisTick: { show: false },
        splitLine: { lineStyle: { type: "dashed", color: ct.splitLineColor } },
      },
      series: [
        {
          type: "line",
          ...
          symbol: "none",
          lineStyle: {
            color: MODULE_COLORS[selectedModule] || "#1976d2",
            width: 2,
          },
          itemStyle: { color: MODULE_COLORS[selectedModule] || "#1976d2" },
          markArea:
            markAreaData.length > 0 ? { data: [markAreaData[0]] } : undefined,
          markPoint:
  ...
  const tblHeaderSm: React.CSSProperties = {
    ...tblHeader,
    padding: "4px 10px",
  };
  const tblRowEven: React.CSSProperties = {
    borderBottom: `1px solid ${ct.tableBorder}`,
    background: ct.tableRowEven,
    cursor: "pointer",
  };
  const tblRowOdd: React.CSSProperties = {
    borderBottom: `1px solid ${ct.tableBorder}`,
        ...
        <Tabs
          ...
          sx={{
            ...
            "& .MuiTab-root": {
              ...
              borderRadius: 0,
              border: `1px solid ${ct.tableBorder}`,
              color: "text.primary",
              minWidth: 160,
            },
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                sx={{
                  ...
                  bgcolor:
                    alertCoverage.remaining === 0 && alertCoverage.total > 0
                      ? darkMode
                        ? SEVERITY_BG_DARK.NORMAL
                        : SEVERITY_BG_LIGHT.NORMAL
                      : darkMode
                      ? SEVERITY_BG_DARK.WARNING
                      : SEVERITY_BG_LIGHT.WARNING,
                  color:
          ...
          {batchRunning && batchProgress && (
            <Paper
              sx={{
                p: 1.5,
                borderRadius: 0,
                bgcolor: darkMode ? "rgba(21,101,192,0.15)" : "#e3f2fd",
                border: `1px solid ${darkMode ? "#1565c0" : "#90caf9"}`,
              }}
            >
              <Box
                sx={{
              ...
              >
                ...
                {batchProgress.leftAfterCap > 0 && (
                  ...
                  >
                    +{batchProgress.leftAfterCap} more queued after this batch
                    (cap {BATCH_CAP}/run)
                  </Typography>
                )}
              </Box>
              <LinearProgress
                ...
                sx={{
                  height: 6,
                  borderRadius: 0,
                  bgcolor: darkMode ? "rgba(21,101,192,0.3)" : "#bbdefb",
            ...
            >
              ...
              >
                {batchResult.analyzed === 0 && batchResult.leftAfterCap === 0
                  ...
                  : `${batchResult.analyzed} new alert${
                      batchResult.analyzed !== 1 ? "s" : ""
                    } analyzed.`}
                {batchResult.failed > 0 &&
                  ` ${batchResult.failed} failed (service error).`}
                {batchResult.leftAfterCap > 0 && (
                  <span
                    style={{
                  ...
                  >
                    {` ${batchResult.leftAfterCap} alert${
                      batchResult.leftAfterCap !== 1 ? "s" : ""
                    } not processed this run (cap ${BATCH_CAP}/run) — click LOAD LATEST again to continue.`}
                  </span>
                )}
              </Typography>
            </Paper>
              ...
              [
                {
                  ...
                  value: fleetKpis.totalRuns,
                  color: "#1976d2",
                },
                {
                  label: "UNIQUE FAULT CODES SEEN",
                  value: fleetKpis.uniqueFaults,
                  color: "#7b1fa2",
                },
                {
                  label: "CRITICAL FAULT TRIGGERS",
                  value: fleetKpis.criticalCount,
                  color: theme.palette.error.main,
                },
                {
                  label: "TOP TRIGGERED CODE",
                  value: fleetKpis.topCode,
                  color: theme.palette.warning.main,
                },
          ...
          <Paper sx={{ p: 1.5, borderRadius: 0 }}>
            ...
            >
              ...
              >
                ...
                <tbody>
                  {perVehicleSummary.length > 0 ? (
                    perVehicleSummary.map((row, i) => (
                      <tr
                        key={row.vehicle_id}
                        onClick={() =>
                          navigateToDeepDive(row.vehicle_id, selectedModule, "")
                        }
                        style={i % 2 === 0 ? tblRowEven : tblRowOdd}
                        title="Click to open Deep Dive for this vehicle"
                      >
                        <td style={{ padding: "5px 12px", fontWeight: 700 }}>
                          {row.vehicle_id}
                        </td>
                        <td
                          style={{
                          ...
                          {row.topCode === "—" ? (
                            ...
                            <span
                              style={{
                                ...
                                fontSize: "10px",
                                fontWeight: 700,
                                background: sevColor(row.topSev),
                                color: "white",
                              }}
                            >
                    ...
                    <tr>
                      ...
                      >
                        {allHistoryQuery.isLoading
                          ? "Loading…"
                          : "No analyses yet — click LOAD LATEST to analyze active alerts"}
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <Box sx={{ display: "flex", gap: 2, height: 400 }}>
            ...
            >
              ...
              >
                HISTORICAL FAULT FREQUENCY — most triggered codes across all
                runs
              </Typography>
                    ...
                    >
                      ...
                      <Tooltip
                        ...
                        formatter={(v: number, _name: string, props: any) => [
                          `${v} runs · ${props.payload.vehicle_count} vehicle(s)`,
                    ...
                    {(allHistoryQuery.data?.runs || []).map(
                      (run: any, i: number) => (
                        <tr
                          ...
                          style={
                            i % 2 === 0
                              ? { ...tblRowEven, padding: undefined }
                              : { ...tblRowOdd, padding: undefined }
                          }
                          title="Click to open Deep Dive for this run"
                        >
                          <td
                            style={{ padding: "4px 10px", color: ct.axisColor }}
                          >
                    ...
                    {!allHistoryQuery.data?.runs?.length && (
                      <tr>
                        ...
                        >
                          {allHistoryQuery.isLoading
                            ? "Loading…"
                            : "No runs yet — click LOAD LATEST to analyze active alerts"}
                        </td>
                      </tr>
                    )}
                  </tbody>
              ...
              >
                {analysisData?.success === true &&
                  analysisTriggered.length === 0 && (
                    <Chip
                      ...
                      sx={{
                        borderRadius: 0,
                        bgcolor: darkMode ? SEVERITY_BG_DARK.NORMAL : "#e8f5e9",
                        color: theme.palette.success.main,
                        fontWeight: "bold",
                        fontSize: "10px",
                        height: 20,
                ...
                {analysisTriggered.length > 0 && (
                  <>
                    <Typography
                      variant="caption"
                      sx={{ color: "text.disabled", fontSize: "10px", mr: 0.5 }}
                    >
                      TRIGGERED:
                    </Typography>
                    {analysisTriggered.map((t: any, i: number) => (
                      <Chip
                        ...
                        sx={{
                          ...
                          border: `1px solid ${sevColor(t.severity)}`,
                        }}
                      />
                    ))}
                  </>
            ...
            {dtcAnalysisQuery.isFetching ? (
              ...
              >
                <Typography
                  ...
                  sx={{ color: "text.disabled", fontFamily: "monospace" }}
                >
                  Select vehicle + module + peak timestamp, then click RUN DTC
                  ANALYSIS
                </Typography>
                <Typography
                  variant="caption"
                ...
                >
                  Runs PyTorch ML models · produces critical and non-critical
                  fault maturation curves
                </Typography>
              </Box>
            )}
          </Paper>
            ...
            >
              ...
              >
                <Typography
                  ...
                  sx={{ fontWeight: "bold", color: "text.secondary" }}
                >
                  VEHICLE ANALYSIS HISTORY — {selectedVehicle || "—"} /{" "}
                  ...
                  <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
                    ...
                    <Chip
                      ...
                      sx={{
                        ...
                        fontWeight: "bold",
                        fontSize: "10px",
                        bgcolor: sevColor(selectedDtcDetail.severity),
                        color: "white",
                        height: 18,
                      }}
                    />
                    <Chip
                      ...
                      sx={{
                        ...
                        fontWeight: "bold",
                        fontSize: "10px",
                        bgcolor: darkMode ? "rgba(25,118,210,0.2)" : "#e3f2fd",
                        color: "#1976d2",
                        height: 18,
                      }}
                    />
                  ...
                  {selectedDtcDetail.features?.length > 0 && (
                    <Box>
                      ...
                      <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                        {(selectedDtcDetail.features as string[]).map(
                          (f: string) => (
                            <Chip
                              key={f}
                              size="small"
                      ...
                      >
                        click feature chip to load sensor evidence
                      </Typography>
                    </Box>
                  )}
                </Box>
          ...
          >
            ...
            >
              <Typography
                ...
                sx={{ fontWeight: "bold", color: "text.secondary" }}
              >
                SENSOR EVIDENCE — {selectedVehicle} /{" "}
                {selectedModule.toUpperCase()}
                {peakTs && (
                  <span
                    style={{ color: theme.palette.error.main, marginLeft: 6 }}
                  >
            ...
            <Box sx={{ flex: 1, minHeight: 0 }}>
              <EChart
                ...
                empty={
                  evidenceData.length === 0 && !sensorEvidenceQuery.isFetching
                }
                emptyText={
                  loadEvidence
