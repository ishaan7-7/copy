...
def _fetch_traceback(source_id: str, module: str, peak_ts: pd.Timestamp) -> pd.DataFrame:
    ...
    if not partition_path.exists():
        partition_path = ROOT_DIR / "data" / "batch" / "bronze" / module / f"source_id={source_id}"
    if not partition_path.exists():
        return pd.DataFrame()
    try:
        pq_files = []
...
async def analyze_dtc(module: str, source_id: str, peak_ts: str) -> dict:
    ...
    try:
        ...
        if result.get("success"):
            if len(_dtc_cache) >= 300:
                _dtc_cache.pop(next(iter(_dtc_cache)))
