...
function MapController({
  ...
  useEffect(() => {
    ...
    map.stop();
    map.setView([22.9937, 78.9629], 4, { animate: true });
  ...
  useEffect(() => {
    ...
    if (hasTrip && !hadTripRef.current) {
      map.stop();
      map.fitBounds(L.latLngBounds(tripPolyline as [number, number][]), {
        padding: [40, 40],
        animate: true,
      });
    }
    hadTripRef.current = hasTrip;
  }, [tripPolyline, map]);
...
export default function CockpitView({
  ...
  const compactNumberFieldSx = {
    ...
    "& .MuiOutlinedInput-input": { fontSize: "9.5px", padding: "3px 2px", textAlign: "center" as const },
  };
  ...
  const { data: automotiveFleetSummary } = useQuery({
    ...
    refetchInterval: isActive && autoRefresh ? 30000 : false,
    staleTime: 15000,
  });
  const { data: dtcFleetDistribution } = useQuery({
  ...
  const selectedVehicleAnchor = useMemo(() => {
    ...
    if (vehicleDetail?.lat != null && vehicleDetail?.lng != null) {
      return { lat: vehicleDetail.lat, lng: vehicleDetail.lng };
    }
    return null;
  ...
  const historicalRouteOverlay = useMemo(() => {
    ...
    if (rawWaypoints && rawWaypoints.length >= 2) {
      ...
      const endLat = anchorLat;
      const endLng = anchorLng;
      const distKm = trip.distance_km ?? 0;
      let hash = 0;
      for (const c of selectedVehicle ?? "") hash = (hash * 31 + c.charCodeAt(0)) & 0xffff;
      const angleRad = ((hash % 360) * Math.PI) / 180;
      const dLat = (distKm / 111) * Math.cos(angleRad);
      const dLng = (distKm / (111 * Math.cos((endLat * Math.PI) / 180))) * Math.sin(angleRad);
      polyline = [[endLat - dLat, endLng - dLng], [endLat, endLng]];
    }
    const events = (lastTripData.trip_events ?? []).map((e) => ({
      lat: e.lat ?? polyline[0][0],
      lng: e.lng ?? polyline[0][1],
      type: e.type ?? "harsh_braking",
  ...
  const maintenanceVehicleBuckets = useMemo(() => {
    ...
    return buckets;
  }, [allPositions, pipelineHealthMap]);
      ...
      >
        ...
        >
          <Box sx={{ flexGrow: 1, minHeight: 0, height: "100%" }}>
                  ...
                  >
                    ...
                    >
                      ...
                      <MapController
                        ...
                        livePosition={selectedVehicleAnchor}
                        isActive={isActive}
                        tripPolyline={historicalRouteOverlay?.polyline ?? null}
                      ...
                      {historicalRouteOverlay && (
                        <>
                          ...
                          {showHistTripEvents && historicalRouteOverlay.events.map((evt, i) => (
                            ...
                            >
                              <Popup>
                                <strong>{evt.type.replace(/_/g, " ").toUpperCase()}</strong>
                                <br />
                                Speed: {evt.speed_kmh} km/h
                                <br />
                    ...
                    <Tooltip title="Reset map view" arrow>
                      <IconButton
                        size="small"
                        onClick={() => setMapResetToken((v) => v + 1)}
                        sx={{
                          position: "absolute",
                          top: 122,
                          right: 10,
            ...
            <Dialog
              open={openMaintenanceForecast}
              onClose={() => setOpenMaintenanceForecast(false)}
              fullWidth
              maxWidth="sm"
            >
              <Box
              ...
              >
                <Stack direction="row" alignItems="center" spacing={1}>
                  <BuildOutlinedIcon sx={{ fontSize: 18, color: "#38bdf8" }} />
                  <Typography sx={{ fontSize: "14px", fontWeight: 700 }}>
                    Maintenance Forecast
                  </Typography>
                </Stack>
                <IconButton
                  onClick={() => setOpenMaintenanceForecast(false)}
                  size="small"
                >
                  <CloseIcon fontSize="small" />
                </IconButton>
              </Box>
              <DialogContent sx={{ p: 0, maxHeight: "75vh" }}>
                <Box
                  sx={{
                    px: 2,
                    py: 1.25,
                    bgcolor: isDark ? alpha("#38bdf8", 0.08) : alpha("#38bdf8", 0.06),
                    borderBottom: `1px solid ${isDark ? "#1e293b" : "#e2e8f0"}`,
                  }}
                >
                ...
                {maintenanceRows.map((row) => {
                  const vehicles = maintenanceVehicleBuckets[row.bucketKey];
                  return (
                    <Accordion
                      key={row.label}
                      defaultExpanded={row.bucketKey === "within_1_week" && vehicles.length > 0}
                      disableGutters
                      sx={{
                        "&:before": { display: "none" },
                        borderBottom: `1px solid ${isDark ? "#1e293b" : "#e2e8f0"}`,
                    ...
                    >
                      ...
                      <AccordionDetails sx={{ px: 2, py: 0.5 }}>
                        <Typography
                          sx={{ fontSize: 10, color: "text.secondary", mb: 1, display: "block" }}
                        >
                          {row.reason}
                        </Typography>
                        {vehicles.length === 0 ? (
                          <Typography sx={{ fontSize: 11, color: "text.secondary", py: 1 }}>
                            No vehicles currently in this window.
                          </Typography>
                        ) : (
                            ...
                            {vehicles.map((v) => (
                              <Box
                                ...
                                sx={{
                                  ...
                                  py: 0.6,
                                  borderRadius: 1,
                                  bgcolor: isDark ? alpha("#1e293b", 0.5) : alpha("#f1f5f9", 0.7),
                                }}
                              >
                                <Box sx={{ minWidth: 0 }}>
                                  ...
                                  <Typography sx={{ fontSize: 9, color: "text.secondary" }} noWrap>
                                    {v.vehicle_id} · {v.type}
                                  </Typography>
                                </Box>
                                <Chip
                                  ...
                                  sx={{
                                    ...
                                    bgcolor: alpha(
                                      v.status === "active" ? "#22c55e" : v.status === "in_service" ? "#8b5cf6" : "#3b82f6",
                                      0.14
                                    ),
                                    color:
                                      v.status === "active" ? "#22c55e" : v.status === "in_service" ? "#8b5cf6" : "#3b82f6",
                                ...
                                />
                                <Typography
                                  sx={{ fontSize: 12, fontWeight: 800, color: row.color, minWidth: 34, textAlign: "right" }}
                                >
                                  {Math.round(v.liveHealth)}%
                                </Typography>
                              </Box>
                            ))}
                          </Stack>
                ...
                >
                  {(() => {
                    ...
                    if (vehicleDetail.status !== "active") {
                      ...
                      const dScore = lastTripData?.driver_summary?.score ?? vehicleDetail.driver_score ?? 0;
                      const dScoreColor =
                        dScore >= 80
                          ? "#22c55e"
                          : dScore >= 60
                        ...
                        <>
                          ...
                          {(vehicleDetail.road_type ||
                            ...
                            <Box sx={{ px: 1.5, py: 1.25 }}>
                              ...
                              >
                                {vehicleDetail.status === "in_service" ? "Service Location" : vehicleDetail.status === "parked" ? "Parked Location" : "Last Known Location"}
                              </Typography>
                              <Stack spacing={0.5}>
                                {vehicleDetail.road_type && (
                                  <Stack
              ...
              >
                ...
                >
                  <Box sx={{ flexShrink: 0 }}>
                    ...
                    >
                      {showWorstPerformers
                        ? "Vehicles needing the most attention, by live health and driver score"
                        : "Real-time overview"}
                    </Typography>
                  </Box>
                  <Stack
                  ...
                  >
                    ...
                    {showWorstPerformers && (
                      <FormControl size="small" sx={{ minWidth: 142 }}>
                        ...
                        <Select
                          label="Worst By"
                          value={worstPerformerSort}
                          onChange={(e) => {
                            setWorstPerformerSort(
                              e.target.value as typeof worstPerformerSort
                            );
                            setPage(0);
                          }}
                          sx={{
                        ...
                        >
                          ...
                          <Stack direction="row" spacing={0.5} alignItems="center">
                            <TextField
                              size="small"
                              type="number"
                              value={healthRangeDraft[0]}
                              onChange={(e) => {
                                const v = clampRangeInput(e.target.value, healthRangeDraft[1], true);
                                const next: [number, number] = [v, healthRangeDraft[1]];
                                setHealthRangeDraft(next);
                                setHealthRangeFilter(next);
                                setPage(0);
                              }}
                            ...
                            <Slider
                              ...
                              min={0}
                              max={100}
                              sx={{ color: "#3b82f6", mx: 0.25 }}
                            />
                            <TextField
                              size="small"
                              type="number"
                              value={healthRangeDraft[1]}
                              onChange={(e) => {
                                const v = clampRangeInput(e.target.value, healthRangeDraft[0], false);
                                const next: [number, number] = [healthRangeDraft[0], v];
                                setHealthRangeDraft(next);
                                setHealthRangeFilter(next);
                                setPage(0);
                              }}
                          ...
                          <Button
                            ...
                            sx={{ fontSize: "9px", textTransform: "none", mt: 0.5, minHeight: 22, py: 0.25 }}
                          >
                            Reset
                          </Button>
                        </Popover>
                    ...
                    >
                      <Typography
                        sx={{
                          fontSize: 10,
                          fontWeight: 800,
                          color: showWorstPerformers
                            ? "#ef4444"
                            : "text.secondary",
                          whiteSpace: "nowrap",
                        }}
                      >
                        Switch to Worst Performing Vehicles
                      </Typography>
                      <Switch
                        size="small"
                        checked={showWorstPerformers}
                        onChange={(e) => {
                          setShowWorstPerformers(e.target.checked);
                          setPage(0);
                        }}
                      />
                      <Tooltip title={tableExpanded ? "Collapse table" : "Expand table"} arrow>
                        <IconButton
                          size="small"
                          ...
                          sx={{
                            p: 0.5,
                            ml: 0.25,
                            color: tableExpanded ? "#3b82f6" : "text.secondary",
                            bgcolor: tableExpanded
                ...
                >
                  {colFilterAnchor?.colId === "name" && (
                    <Box>
                      <Typography sx={{ fontSize: "8.5px", fontWeight: 800, color: "text.secondary", textTransform: "uppercase", letterSpacing: ".04em", mb: 0.4 }}>
                        Sort
                      </Typography>
                      <Stack direction="row" spacing={0.4} sx={{ mb: 0.75 }}>
                        <Button size="small" variant={orderBy === "name" && order === "asc" ? "contained" : "outlined"} onClick={() => setColSort("name", "asc")} sx={{ flex: 1, fontSize: "8px", textTransform: "none", minHeight: 22, py: 0.25 }}>
                          A → Z
                        </Button>
                        <Button size="small" variant={orderBy === "name" && order === "desc" ? "contained" : "outlined"} onClick={() => setColSort("name", "desc")} sx={{ flex: 1, fontSize: "8px", textTransform: "none", minHeight: 22, py: 0.25 }}>
                          Z → A
                        </Button>
                      </Stack>
                      <Typography sx={{ fontSize: "8.5px", fontWeight: 800, color: "text.secondary", textTransform: "uppercase", letterSpacing: ".04em", mb: 0.3 }}>
                        Search
                      </Typography>
                      <TextField
                        size="small"
                        ...
                        sx={{ mb: 0.75, "& .MuiOutlinedInput-root": { fontSize: "9.5px", height: 24 } }}
                      />
                      <Typography sx={{ fontSize: "8.5px", fontWeight: 800, color: "text.secondary", textTransform: "uppercase", letterSpacing: ".04em", mb: 0.1 }}>
                        Status
                      </Typography>
                      {[
                        ...
                        .map((s) => (
                          <FormControlLabel
                            key={s.key}
                            sx={{ display: "flex", ml: 0, mr: 0, minHeight: 20 }}
                            control={
                              <Checkbox
                                size="small"
                            ...
                            label={<Typography sx={{ fontSize: "9.5px" }}>{s.label}</Typography>}
                          />
                        ))}
                      <Button
                        size="small"
                        ...
                        sx={{ mt: 0.5, fontSize: "8.5px", textTransform: "none", minHeight: 22, py: 0.25 }}
                      >
                        Reset
                      </Button>
                    </Box>
                  ...
                  {colFilterAnchor?.colId === "type" && (
                    <Box>
                      ...
                      <Stack direction="row" spacing={0.4} sx={{ mb: 0.75 }}>
                        <Button size="small" variant={orderBy === "type" && order === "asc" ? "contained" : "outlined"} onClick={() => setColSort("type", "asc")} sx={{ flex: 1, fontSize: "8px", textTransform: "none", minHeight: 22, py: 0.25 }}>
                          A → Z
                        </Button>
                        <Button size="small" variant={orderBy === "type" && order === "desc" ? "contained" : "outlined"} onClick={() => setColSort("type", "desc")} sx={{ flex: 1, fontSize: "8px", textTransform: "none", minHeight: 22, py: 0.25 }}>
                          Z → A
                        </Button>
                      </Stack>
                      <Typography sx={{ fontSize: "8.5px", fontWeight: 800, color: "text.secondary", textTransform: "uppercase", letterSpacing: ".04em", mb: 0.1 }}>
                      ...
                      {Array.from(availableTypeOptions)
                        ...
                        .map((t) => (
                          <FormControlLabel
                            ...
                            label={<Typography sx={{ fontSize: "9.5px" }}>{t}</Typography>}
                          />
                        ))}
                      {availableTypeOptions.size === 0 && (
                        <Typography sx={{ fontSize: "9px", color: "text.secondary", py: 0.5 }}>
                  ...
                  {colFilterAnchor?.colId === "health_status" && (
                    <Box>
                      ...
                      <Stack direction="row" spacing={0.4} sx={{ mb: 0.75 }}>
                        <Button size="small" variant={orderBy === "health_status" && order === "asc" ? "contained" : "outlined"} onClick={() => setColSort("health_status", "asc")} sx={{ flex: 1, fontSize: "8px", textTransform: "none", minHeight: 22, py: 0.25 }}>
                          Worst → Best
                        </Button>
                        <Button size="small" variant={orderBy === "health_status" && order === "desc" ? "contained" : "outlined"} onClick={() => setColSort("health_status", "desc")} sx={{ flex: 1, fontSize: "8px", textTransform: "none", minHeight: 22, py: 0.25 }}>
                          Best → Worst
                        </Button>
                      </Stack>
                      <Typography sx={{ fontSize: "8.5px", fontWeight: 800, color: "text.secondary", textTransform: "uppercase", letterSpacing: ".04em", mb: 0.1 }}>
                      ...
                      {["Excellent", "Good", "Average", "Poor"]
                        ...
                        .map((s) => (
                          <FormControlLabel
                            ...
                            label={<Typography sx={{ fontSize: "9.5px" }}>{s}</Typography>}
                          />
                        ))}
                      <Button
                        size="small"
                  ...
                  {colFilterAnchor?.colId === "health" && (
                    <Box>
                      ...
                      <Stack direction="row" spacing={0.4} sx={{ mb: 0.75 }}>
                        <Button size="small" variant={orderBy === "health" && order === "asc" ? "contained" : "outlined"} onClick={() => setColSort("health", "asc")} sx={{ flex: 1, fontSize: "8px", textTransform: "none", minHeight: 22, py: 0.25 }}>
                          Low → High
                        </Button>
                        <Button size="small" variant={orderBy === "health" && order === "desc" ? "contained" : "outlined"} onClick={() => setColSort("health", "desc")} sx={{ flex: 1, fontSize: "8px", textTransform: "none", minHeight: 22, py: 0.25 }}>
                          High → Low
                        </Button>
                      </Stack>
                      <Typography sx={{ fontSize: "8.5px", fontWeight: 800, color: "text.secondary", textTransform: "uppercase", letterSpacing: ".04em", mb: 0.5 }}>
                  ...
                  {colFilterAnchor?.colId === "driver_score" && (
                    <Box>
                      ...
                      <Stack direction="row" spacing={0.4} sx={{ mb: 0.75 }}>
                        <Button size="small" variant={orderBy === "driver_score" && order === "asc" ? "contained" : "outlined"} onClick={() => setColSort("driver_score", "asc")} sx={{ flex: 1, fontSize: "8px", textTransform: "none", minHeight: 22, py: 0.25 }}>
                          Low → High
                        </Button>
                        <Button size="small" variant={orderBy === "driver_score" && order === "desc" ? "contained" : "outlined"} onClick={() => setColSort("driver_score", "desc")} sx={{ flex: 1, fontSize: "8px", textTransform: "none", minHeight: 22, py: 0.25 }}>
                          High → Low
                        </Button>
                      </Stack>
                      <Typography sx={{ fontSize: "8.5px", fontWeight: 800, color: "text.secondary", textTransform: "uppercase", letterSpacing: ".04em", mb: 0.5 }}>
