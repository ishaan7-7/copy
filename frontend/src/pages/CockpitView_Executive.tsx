...
function ScoreTimeline({ data }: { data: BehaviorData }) {
  ...
  const option = {
    ...
    xAxis: {
      ...
      data: sampled.map((t) => convertDistance(t.distance_km, region)),
      name: distanceUnitLabel(region),
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: (p: { data: number[] }) =>
        `Lat: ${p.data[0]}g  Lon: ${p.data[1]}g  Spd: ${formatSpeed(p.data[2], region, 0)}`,
    },
    xAxis: {
      type: "value" as const,
      min: -1.2,
    ...
    series: [
      {
        ...
        data: roads.map(([, s]) => [s.min, s.q1, s.median, s.q3, s.max].map((v) => convertSpeed(v, region))),
      ...
      {sampled.map((evt, i) => (
        ...
        >
          <Popup>
            ...
            <br />
            Speed: {formatSpeed(evt.speed, region, 0)}
          </Popup>
        </Marker>
      ))}
    </>
...
const REGION_MAP_CENTER: Record<Region, [number, number]> = {
  india: [22.9937, 78.9629],
  america: [39.8283, -98.5795],
};
const REGION_MAP_ZOOM: Record<Region, number> = {
  india: 4,
  america: 4,
};
...
function MapController({
  ...
  useEffect(() => {
    if (!didMountRegionRef.current) {
      didMountRegionRef.current = true;
      return;
    }
    lastFittedVehicle.current = null;
    hadTripRef.current = false;
  ...
  useEffect(() => {
    ...
    if (!selectedVehicle) {
      lastFittedVehicle.current = null;
      map.stop();
      map.flyTo(REGION_MAP_CENTER[region], REGION_MAP_ZOOM[region], { duration: 0.6 });
    }
...
function SpeedGauge({ speed, isDark }: { speed: number; isDark: boolean }) {
  const region = useStore((s) => s.region);
  const displaySpeed = convertSpeed(speed, region);
  const maxSpeed = convertSpeed(120, region);
  const highThreshold = convertSpeed(90, region);
  const midThreshold = convertSpeed(60, region);
  const r = 15,
    cx = 19,
    cy = 19;
  const halfLen = Math.PI * r;
  const pct = Math.min(displaySpeed / maxSpeed, 1);
  const color = displaySpeed > highThreshold ? "#ef4444" : displaySpeed > midThreshold ? "#f59e0b" : "#22c55e";
...
export default function CockpitViewExecutive({
  ...
  const prefetchVehicleSummary = (vehicleId: string) => {
    queryClient.prefetchQuery({
      ...
      queryFn: () => axios.get(`${PIPELINE_API}/api/automotive/vehicle-summary/${vehicleId}`).then((r) => r.data),
    });
  };
  ...
  const region = useStore((s) => s.region);
  const setRegionStore = useStore((s) => s.setRegion);
  const queryClient = useQueryClient();

  const regionMutation = useMutation({
    mutationFn: (newRegion: Region) =>
      axios.post(`${FLEET_API}/region`, { region: newRegion }).then((r) => r.data),
    onSuccess: (_data, newRegion) => {
      setRegionStore(newRegion);
      queryClient.invalidateQueries({ queryKey: ["fleet-summary"] });
      queryClient.invalidateQueries({ queryKey: ["fleet-positions"] });
      queryClient.invalidateQueries({ queryKey: ["fleet-vehicle"] });
      queryClient.invalidateQueries({ queryKey: ["fleet-trip"] });
      queryClient.invalidateQueries({ queryKey: ["fleet-behavior"] });
      ...
      >
        ...
        >
          <Box sx={{ flexGrow: 1, minHeight: 0, height: "100%" }}>
                          ...
                          >
                            <Box sx={{ minWidth: 150 }}>
                              ...
                              <Typography sx={{ fontSize: 10 }}>
                                ...
                                {formatSpeed(v.speed ?? 0, region, 0)}
                              </Typography>
                              <Typography sx={{ fontSize: 10 }}>
                          ...
                          {showHistTripEvents && historicalRouteOverlay.events.map((evt, i) => (
                            ...
                            >
                              <Popup>
                                ...
                                <br />
                                Speed: {formatSpeed(evt.speed_kmh, region, 0)}
                                <br />
            ...
            <Dialog
              open={isActive && openMaintenanceForecast}
            ...
            <Dialog
              open={isActive && openHealthScorePopup}
            ...
            <Dialog
              open={isActive && openDriverScorePopup}
            ...
            <VehicleSummaryPopup
              open={isActive && !!vehicleSummaryPopupId}
            ...
            <Popover
              open={isActive && Boolean(selectedVehicle && popoverPosition)}
                            ...
                            >
                              ...
                              <Stack spacing={0.6}>
                                ...
                                >
                                  ...
                                  >
                                    {formatDistance(lastTripData.last_trip.distance_km, region)}
                                  </Typography>
                                </Stack>
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    m ·{" "}
                                    {formatSpeed(lastTripData.last_trip.avg_speed_kmh, region, 0)} avg
                                  </Typography>
                                </Stack>
                                    ...
                                    >
                                      ...
                                      >
                                        <Box>
                                          ...
                                          >
                                            {formatDistance(tripData.distance_completed_km, region)}
                                          </Typography>
                                        ...
                                        <Box sx={{ textAlign: "right" }}>
                                          ...
                                          >
                                            {formatDistance(tripData.distance_total_km, region)}
                                          </Typography>
                                    ...
                                    >
                                      ...
                                      >
                                        {(() => {
                                          ...
                                          const speed = convertSpeed(rawSpeed, region);
                                          const percentage = rawSpeed / gaugeMaxKmh;
                                      ...
                                      {tripData.events
                                        ...
                                        .map((e, i) => (
                                          ...
                                          >
                                            ...
                                            >
                                              {e.type.toUpperCase()} •{" "}
                                              {formatDistance(e.distance_km, region)} • {formatSpeed(e.speed, region, 0)} • G(
                                              {e.acc_x.toFixed(2)})
                                            </Typography>
                                          </Box>
                                        ))}
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      >
                                        {formatDistance(behaviorData.trip_distance_km, region)}
                                      </Typography>
                                    </Box>
                                  </Box>
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        {/* EXPANDED MODAL */}
                        <Dialog
                          open={isActive && openFleetScatter}
                  ...
                  {(() => {
                        ...
                        formatter: (params: any) => {
                          ...
                          const xLabel =
                            timelineXAxis === "mileage"
                              ? `${Math.round(
                                  convertDistance(Number(rawX), region)
                                ).toLocaleString()} ${distanceUnitLabel(region)}`
                        ...
                        axisLabel: {
                          ...
                          formatter: (v: any) => {
                            if (timelineXAxis === "mileage") {
                              const dist = Math.round(convertDistance(Number(v), region));
                              return dist >= 1000 ? `${(dist / 1000).toFixed(1)}k` : `${dist}`;
                            }
                                ...
                                ? [
                                    {
                                      ...
                                      label: {
                                        formatter: `NEXT SVC\n+${Math.round(
                                          convertDistance(SERVICE_INTERVAL_KM - kmSinceService, region)
                                        ).toLocaleString()} ${distanceUnitLabel(region)}`,
                    ...
                    return (
                      <>
                        ...
                        <Dialog
                          open={isActive && openVehicleHealth}
                ...
                >
                  ...
                  <Card
                    sx={{
                      ...
                      background: isDark
                        ? `linear-gradient(145deg, ${alpha("#2563eb", 0.12)}, rgba(15,23,42,0.92))`
                          ...
                          >
                            <Box
                              sx={{
                                width: `${Math.max(
                                  row.value > 0 ? 8 : 0,
                                  (row.value / Math.max(1, ...alertChartRows.map((item) => item.value))) * 100
                                )}%`,
                                height: "100%",
                                borderRadius: 999,
                                bgcolor: row.color,
                        ...
                        .map((row, rowIdx) => {
                          ...
                          return (
                            <TableRow
                              hover
                              key={row.vehicle_id}
                              onMouseEnter={() => prefetchVehicleSummary(row.vehicle_id)}
                              sx={{
                                bgcolor: rowBg,
                                borderBottom: `1px solid ${
                                  isDark ? "#1e293b" : "#e2e8f0"
