...
export default function FleetCenter() {
  ...
  useQuery({
    ...
    queryFn: async () => {
      await Promise.all(
        activeIds.flatMap((vid) => [
          axios
            .get(`${FLEET_API}/vehicle/${vid}`)
            .then((r) => {
              queryClient.setQueryData(["fleet-vehicle", vid], r.data);
            })
            .catch(() => {}),
          axios
            .get(`${FLEET_API}/vehicle/${vid}/trip`)
            .then((r) => {
              queryClient.setQueryData(["fleet-trip", vid], r.data);
            })
            .catch(() => {}),
          axios
            .get(`${FLEET_API}/vehicle/${vid}/behavior`)
            .then((r) => {
              queryClient.setQueryData(["fleet-behavior", vid], r.data);
            })
            .catch(() => {}),
        ])
      );
  ...
  const { data: fleetHealthScatter } = useQuery<
    ...
    queryFn: async () => {
      const results = await Promise.all(
        activeVehicleIds.map((vid) =>
          axios
            .get(`${PIPELINE_API}/api/automotive/vehicle-health-history/${vid}`)
            .then((r) => ({
              vehicle_id: vid,
              data: (r.data.data || []) as HealthHistoryRow[],
            }))
  ...
  return (
    ...
    >
      <style>{`
        ...
        } !important; font-family: Inter, Segoe UI, Roboto, Arial, sans-serif !important; }
        .leaflet-popup-content { font-family: Inter, Segoe UI, Roboto, Arial, sans-serif; font-size: 12px; line-height: 1.5; }
