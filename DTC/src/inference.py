...
from DTC.src.config import PHYSICAL_THRESHOLD_OVERRIDES
...
class DTCInferenceService:
    ...
    def analyze_window(self, df):
        ...
        for dtc_code, model in self.models.items():
            ...
            override = PHYSICAL_THRESHOLD_OVERRIDES.get((self.module_name, dtc_code))
            if override is not None:
                probs = self._physical_override_series(df, override)
                if probs is None:
                    continue
            else:
    ...
    @staticmethod
    def _physical_override_series(df, override):
        """
        Plain threshold check on already-corrected raw sensor values, used in
        place of a frozen ML model whose calibration predates that correction
        (see PHYSICAL_THRESHOLD_OVERRIDES). Returns a per-row 0.0/1.0 series —
        the same [0,1] "probability" shape the rest of the pipeline expects —
        so it flows through the existing buildup/attribution logic unchanged.
        """
        ...
        if override["direction"] == "below":
            breach = pd.concat([df[c] < threshold for c in cols], axis=1).any(axis=1)
        else:
            breach = pd.concat([df[c] > threshold for c in cols], axis=1).any(axis=1)
        return breach.astype(float).values
