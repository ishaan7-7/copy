...
MANIFEST_PATH = ROOT_DIR / "config" / "fleet_manifest.json"
...
def _load_config() -> dict:
    ...
    try:
        with MANIFEST_PATH.open("r", encoding="utf-8") as fh:
            cfg["enabled_sims"] = json.load(fh)["active_sims"]
    except (OSError, KeyError, json.JSONDecodeError) as exc:
        print(f"Warning: could not read fleet manifest ({exc}); "
              f"falling back to enabled_sims from replay_config.json")
    return cfg
...
def _collect_overrides(args: argparse.Namespace) -> dict:
    ...
    if args.rate is not None:
        overrides["rows_per_second"] = args.rate
    if args.sims is not None:
        overrides["enabled_sims"] = [s.strip() for s in args.sims.split(",")]
    if args.mode is not None:
        overrides["replay_mode"] = args.mode
    if args.batch_size is not None:
        overrides["batch_size"] = args.batch_size
    if args.batch_interval is not None:
        overrides["batch_interval_seconds"] = args.batch_interval
    reset_overrides: dict = {}
    if args.with_reset:
        reset_overrides["enabled"] = True
    if args.no_archive_dlq:
        reset_overrides["archive_dlq"] = False
    if reset_overrides:
        overrides["reset"] = reset_overrides
    return overrides


def _merge_overrides(cfg: dict, overrides: dict) -> dict:
    merged = dict(cfg)
    reset_overrides = overrides.get("reset", {})
    for key, value in overrides.items():
        if key != "reset":
            merged[key] = value
    if reset_overrides:
        merged["reset"] = {**cfg.get("reset", {}), **reset_overrides}
    return merged
...
def cmd_start(args: argparse.Namespace) -> None:
    ...
    cfg = _merge_overrides(_load_config(), overrides)
    ...
    if overrides:
        worker_cmd += ["--overrides", json.dumps(overrides)]
...
def cmd_reset(args: argparse.Namespace) -> None:
    ...
    ingest_base = cfg["http_endpoint"].split("/ingest/")[0]
    try:
        import requests
        requests.post(f"{ingest_base}/internal/reset-idempotency", timeout=3)
        print("Ingest idempotency cache cleared.")
    except Exception as e:
        print(f"   ⚠️ Could not clear ingest idempotency cache (ingest may not be running): {e}")
        print("   If ingest was already running before this reset, replayed rows may be")
        print("   silently rejected as duplicates until ingest is restarted.")
...
def _worker_main(overrides_json: str = "") -> None:
    ...
    if overrides_json:
        try:
            cfg = _merge_overrides(cfg, json.loads(overrides_json))
        except (json.JSONDecodeError, TypeError) as exc:
            print(f"Ignoring invalid --overrides payload: {exc}")
    service = ReplayService(
        pipeline_root=ROOT_DIR,
        enabled_sims=cfg.get("enabled_sims"),
        replay_mode=cfg["replay_mode"],
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--overrides",      type=str, default="",            help=argparse.SUPPRESS)
