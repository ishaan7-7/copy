...
export default function AutomotiveDive({ isActive = true }: { isActive?: boolean }) {
  ...
  const sensorQuery = useQuery({
    ...
    queryFn: () =>
      axios
        ...
        .then((r) => r.data),
    enabled: isActive && !!selectedVehicle && deferredTab === "vehicle",
  ...
  const crossfleetQuery = useQuery({
    ...
    queryFn: () =>
      axios
        ...
        .then((r) => r.data),
    enabled: isActive && deferredTab === "module",
    refetchInterval: false,
    placeholderData: undefined,
  });
  ...
  const moduleTimelineQuery = useQuery({
    ...
    queryFn: () =>
      axios
        ...
        .then((r) => r.data),
    enabled: isActive && !!selectedVehicle && deferredTab === "module",
    refetchInterval: false,
    placeholderData: undefined,
  });
  ...
  const moduleFleetHealthQuery = useQuery({
    ...
    queryFn: () =>
      axios
        ...
        .then((r) => r.data),
    enabled: isActive && deferredTab === "module",
    refetchInterval: isActive && deferredTab === "module" && autoRefresh ? 4000 : false,
    placeholderData: undefined,
  });
  ...
  useEffect(() => {
    ...
    setDtcResult(null);
    setDtcRunning(false);
  }, [selectedVehicle, selectedModule]);
  ...
  const runDtcAnalysis = async () => {
    ...
    try {
      ...
      setDtcResult({
        error:
          "DTC service offline or unreachable (port 8007). Start dtc_service/api.py to enable analysis.",
      });
    } finally {
      setDtcRunning(false);
    }
  ...
  const tblRowEven = {
    background: ct.tableBg,
    height: 36,
  };

  const tblRowOdd = {
    background: ct.tableAltBg,
    height: 36,
  };
        ...
        >
          {!fleetQuery.isLoading && vehicles.length === 0 && (
            ...
            >
              ...
              >
                No vehicle data. Start the streaming pipeline to populate fleet
                data.
              </Typography>
            </Box>
          )}
          <Box sx={{ display: "flex", gap: 1, width: "100%" }}>
            {[
              ...
              },
              {
                ...
                color:
                  (fleetStats.avg_health ?? 100) < 60 ? "#d32f2f" : "#2e7d32",
                bg: (fleetStats.avg_health ?? 100) < 60 ? "#ffebee" : "#e8f5e9",
              },
              {
                label: "CRITICAL ( < 60% )",
                value: fleetStats.critical_count ?? 0,
                color:
                  (fleetStats.critical_count ?? 0) > 0 ? "#d32f2f" : "#e53935",
                bg:
                  (fleetStats.critical_count ?? 0) > 0 ? "#ffebee" : "#ffebee",
              },
              {
                label: "WARNING ( 60–80% )",
                value: fleetStats.warning_count ?? 0,
                color:
                  (fleetStats.warning_count ?? 0) > 0 ? "#ed6c02" : "#616161",
                bg: (fleetStats.warning_count ?? 0) > 0 ? "#fff4e5" : "#f5f5f5",
              },
            ].map((kpi, i) => (
              <Paper
                key={i}
          ...
          >
            ...
            <Chip
              ...
              sx={{
                ...
                fontSize: "10px",
                height: 24,
                bgcolor: darkMode ? "rgba(21,101,192,0.2)" : "#e3f2fd",
              }}
            />
          </Paper>
              ...
              >
                ...
                >
                  {kpiFields.map((f) => {
                    ...
                    const isWarn =
                      val !== null && f.warnFn ? f.warnFn(val) : false;
                      ...
                      <Paper
                        ...
                        sx={{
                          ...
                          borderLeft: `4px solid ${
                            isWarn ? "#d32f2f" : MODULE_COLORS[selectedModule]
                          }`,
                        }}
                      >
                        <Typography
                        ...
                        <Typography
                          ...
                          sx={{
                            fontWeight: "bold",
                            color: isWarn ? "#d32f2f" : "text.primary",
                            fontSize: "1.1rem",
                            mt: 0.25,
                          }}
                        >
            ...
            >
              ...
              >
                ...
                <Box sx={{ display: "flex", gap: 1 }}>
                  {[
                    { label: "NORMAL", color: "#2e7d32" },
                    { label: "WARNING", color: "#ed6c02" },
                  ...
                  <tbody>
                    {[
                      ...
                      <tr
                        ...
                        style={i % 2 === 0 ? tblRowEven : tblRowOdd}
                        onMouseEnter={(e) => {
                          e.currentTarget.style.background = ct.gridColor;
                        }}
                        onMouseLeave={(e) => {
                          e.currentTarget.style.background =
                            i % 2 === 0 ? ct.tableBg : ct.tableAltBg;
                        }}
                      >
                        {/* STATUS */}
                        <td style={tblCell}>
          ...
          <Paper sx={{ p: 1.5, borderRadius: 0 }}>
            ...
            {!dtcResult && !dtcRunning && (
              ...
              >
                ...
                dtc_service/api.py running on port 8007.
              </Typography>
            )}
          </Paper>
