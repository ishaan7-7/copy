...
from common.driver_score import P_BRAKE, P_ACCEL, P_CORNER, compute_driver_score
...
def update_behavior(state: BehaviorState, events: dict, distance_m: float, distance_km: float, speed: float, road_type: str, acc_x: float, acc_y: float) -> None:
    ...
    state.score = compute_driver_score(
        state.total_braking, state.total_accel, state.total_cornering, state.total_distance_km
    )
