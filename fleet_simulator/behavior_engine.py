...
def update_behavior(state: BehaviorState, events: dict, distance_m: float, distance_km: float, speed: float, road_type: str, acc_x: float, acc_y: float) -> None:
    ...
    weighted = (
        state.total_braking * P_BRAKE
        + state.total_accel * P_ACCEL
        + state.total_cornering * P_CORNER
    )
    state.score = max(0.0, 100.0 - (weighted / max(state.total_distance_km, 0.1)) * SCORE_NORM)
