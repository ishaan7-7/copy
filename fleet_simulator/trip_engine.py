...
class TripEngine:
    def __init__(self, region: str = "india"):
        ...
        self._route_vehicle_count: dict[str, int] = defaultdict(int)
        self._route_vehicle_rank: dict[str, int] = {}
    ...
    def _build_route_ranks(self) -> None:
        ...
        for v in self.all_vehicles:
            if v["status"] != "active":
                continue
            r = self._route_key_for(v)
            self._route_vehicle_count[r] += 1
        for v in self.all_vehicles:
            if v["status"] != "active":
                continue
            r = self._route_key_for(v)
            self._route_vehicle_rank[v["id"]] = _seen[r]
            _seen[r] += 1

    def initialize(self):
        self.routes = build_all_routes(region=self.region)
        self._start_time = time.time()
        self._build_route_ranks()

        for v in self.all_vehicles:
            if v["status"] != "active":
                continue

            route_key = self._route_key_for(v)
            route = self.routes[route_key]
            rank = self._route_vehicle_rank.get(v["id"], 0)
            n_on_route = self._route_vehicle_count.get(route_key, 1)
            ...
            for i in range(0, n_pts, sample_step):
                ...
                if random.random() < event_chance:
                    ...
                    if any(events.values()):
                        event_fired = True
                        # The score/behavior accumulation above needs a much
                        # longer pseudo-history than one partial pass to
                        # converge (hence looping over the full route
                        # multiple times), but the map only ever draws the
                        # route from its start up to the vehicle's CURRENT
                        # position_index — an event marker placed beyond
                        # that (or from a later wrap-around pass) would show
                        # up sitting on a stretch of road the vehicle hasn't
                        # reached yet, breaking the "recorded as it happens"
                        # read of the highlighted trip. Only the first pass,
                        # only up to position_index, is real map-visible
                        # history; every other backfilled tick still counts
                        # toward the score but stays off the map.
                        if outer_loops == 1 and i <= state.position_index:
    ...
    def get_trip_data(self, vehicle_id: str) -> dict | None:
        ...
        return {
            ...
            # a single forward pass, so the frontend treats the whole route
            # as already-driven once this flips (it pings-pongs from here on).
            "has_reversed": st.has_reversed,
    ...
    def get_behavior_data(self, vehicle_id: str) -> dict | None:
        ...
        return {
            ...
            "event_summary": {
                "braking": beh.leg_braking,
                "accel": beh.leg_accel,
                "cornering": beh.leg_cornering,
                "total": beh.leg_braking + beh.leg_accel + beh.leg_cornering,
            },
