...
def _sync_update_metrics():
    ...
    try:
        ...
        lags = {mod: max(0, silver_counts[mod] - gold_count) for mod in GOLD_ENABLED_MODULES}
        GOLD_METRICS_CACHE["active_sims"] = sorted(list(active_sims))
        GOLD_METRICS_CACHE["total_gold_rows"] = gold_count
        GOLD_METRICS_CACHE["processing_lags"] = lags
        GOLD_METRICS_CACHE["global_max_lag"] = max(lags.values()) if lags else 0

        fleet_health_pct = None
        if gold_path.exists():
            try:
                gfiles_health = dr.list_files(str(gold_path), max_files=_GOLD_MAX_FILES)
                if gfiles_health:
                    health_df = dr.query_df(
                        """SELECT AVG(vehicle_health_score) AS avg_health FROM (
                            SELECT vehicle_health_score,
                                   ROW_NUMBER() OVER (PARTITION BY source_id ORDER BY gold_window_ts DESC) AS rn
                            FROM read_parquet(?)
                        ) t WHERE rn = 1""",
                        gfiles_health,
                    )
                    if not health_df.empty and health_df.iloc[0]["avg_health"] is not None:
                        fleet_health_pct = round(float(health_df.iloc[0]["avg_health"]), 1)
            except Exception:
                pass
        GOLD_METRICS_CACHE["fleet_vehicle_health_pct"] = fleet_health_pct
        ...
        print(f"Gold metrics loop failed, will retry next cycle: {e}")
...
if __name__ == "__main__":
    ...
    uvicorn.run("api:app", host="127.0.0.1", port=8003)