...
def main() -> None:
    ...
    if args.build_frontend:
        node_exe = os.path.join(ROOT, "tools", "node", "node.exe")
        if os.path.exists(node_exe):
            print("\n=== Step 6: building static frontend ===")
            result = subprocess.run([PYTHON, os.path.join(tools, "build_frontend.py")], cwd=ROOT)
            if result.returncode != 0:
                print("FAILED: frontend build — dashboard will still work via Vite dev server.")
            else:
                print("    Built and stamped. run.py will serve it at http://127.0.0.1:8005 and skip Vite,")
                print("    automatically rebuilding whenever frontend source changes.")
        else:
